#!/usr/bin/env Rscript
###############################################################################
## 2_3_cluster.R  --  per-genome final clustering (Part 2, Step 2)
##
...
##
## Usage:
##   Rscript 2_3_cluster.R <soc_rds> <meta_v6_tsv> <outdir> <prefix> \
...
if (length(args) < 9) {
  stop("Usage: Rscript 2_3_cluster.R <soc_rds> <meta_v6_tsv> <outdir> <prefix> <pcs> <k_near> <min_dist> <min_c> <resolution> [seed=1] [genome_label] [m_clst=50]")
}
obj_path   <- args[1]
meta_path  <- args[2]
outdir     <- args[3]
