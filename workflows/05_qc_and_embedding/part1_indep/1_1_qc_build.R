###############################################################################
## 1_1_qc_build.R -- Step 1_1: build the raw Socrates object for one library
...
## Usage: Rscript 1_1_qc_build.R <tn5.bed.gz> <prefix> <gff3|gtf> <chr_sizes> \
##          [genomesize=1.689e9] [org_scaffolds=At_Mt,At_Pt,Zm_Mt,Zm_Pt]
## Called by part1_indep/1_0_qc_run.sh (independent co-embed track); identical to
## common/1_1_QC_scifiATAC_data.R, copied in so the track is self-contained.
###############################################################################
...
# arguments
args <- commandArgs(T)
if(length(args) < 4){stop("Rscript 1_1_qc_build.R <bed> <prefix> <gff/gtf> <chr> [genomesize] [org_scaffolds_comma_sep]")}
