#!/usr/bin/env Rscript
###############################################################################
## 3_0_1_resolution_scan.R  --  per-genome pcs x resolution stability scan
...
##
## Usage:
##   Rscript 3_0_1_resolution_scan.R <soc_rds> <meta_tsv> <outdir> <prefix> \
...
if (length(args) < 7) {
  stop("Usage: Rscript 3_0_1_resolution_scan.R <soc_rds> <meta_tsv> <outdir> <prefix> <pcs_list> <k_near> <min_dist> [min_c=NA] [seed=1] [n_boot=10] [boot_frac=0.8] [res_grid]")
}
soc_rds   <- args[1]
meta_tsv  <- args[2]
outdir    <- args[3]
