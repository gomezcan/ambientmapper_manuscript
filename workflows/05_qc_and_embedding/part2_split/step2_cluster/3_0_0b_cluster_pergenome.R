#!/usr/bin/env Rscript
###############################################################################
## 3_0_0b_cluster_pergenome.R  --  per-genome final clustering (Part 2, Step 2)
##
...
##
## Usage:
##   Rscript 3_0_0b_cluster_pergenome.R <soc_rds> <meta_v6_tsv> <outdir> <prefix> \
...
if (length(args) < 9) {
  stop("Usage: Rscript 3_0_0b_cluster_pergenome.R <soc_rds> <meta_v6_tsv> <outdir> <prefix> <pcs> <k_near> <min_dist> <min_c> <resolution> [seed=1] [genome_label] [m_clst=50]")
}
obj_path   <- args[1]
meta_path  <- args[2]
outdir     <- args[3]
...
out_dir_plots <- file.path(outdir, "plots/step2")
dir.create(out_dir_plots, showWarnings = FALSE, recursive = TRUE)
out_dir_obj <- file.path(outdir, "step2_cluster")            # v7 objects live under step2_cluster/
...
obj$meta$Genome <- genome_lab                        # single-genome constant (no species normalization)
